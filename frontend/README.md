# CertiFlow — Developer Setup Guide
...
### Is it possible?
**Yes, absolutely.** CertiFlow works by using Google OAuth 2.0.
...
2. Create a new project: "CertiFlow"
3. Enable these APIs:
   - Gmail API
   - Google Drive API
   - Google Sheets API
...
function triggerGoogleOAuth() {
  const type = sessionStorage.getItem('certiflow_account_type') || 'personal';
...
if (token) {
  localStorage.setItem('certiflow_token', token);
...
For questions about building CertiFlow, open an issue or refer to:
