...
function validateStep3() {
  ...
  // ... inside validateStep3() ...
    ...
    // THE FIX: Add 'true' to force the pipeline forward
    goStep(4, true); 
}
/* ── File Naming Logic ─────────────────────────────── */
...
/* ════════════════════════════════════════════════════════════════
   STEP 4 — PIPELINE GLUE (Init & Tags)
════════════════════════════════════════════════════════════════ */
function initStep4() {
  ...
  // 2. Initialize CodeMirror (Raw HTML Editor)
  ...
  if (cmEl && typeof CodeMirror !== 'undefined') {
    ...
    ME.cm.on('change', () => {
      clearTimeout(ME.cmDebounce);
      ME.cmDebounce = setTimeout(() => {
        if (ME.activeTab === 'code') {
          const frame = document.getElementById('mPrvBoxS3');
          if (frame) { const doc = frame.contentDocument || frame.contentWindow.document; doc.open(); doc.write(ME.cm.getValue()); doc.close(); }
        }
      }, 500);
    });
  }

  // 3. Setup default blocks & tags
  if (ME.blocks.length === 0) { 
      mAddBlock('logo'); 
      mAddBlock('text'); 
  }
...
// Overwrite the original tag populator to use CP.headers
function mPopulateTags() {
  ...
  // Add the vital Certificate Link tag (This links the PDF to the email!)
  html += `
    <div onclick="mInsertTag('{{Certificate Link}}')" style="padding:8px 12px; margin-top:8px; background:rgba(16,185,129,0.08); border:1px solid rgba(16,185,129,0.3); border-radius:6px; cursor:pointer; font-family:var(--font-mono); font-size:12px; color:var(--green); transition:0.2s;" onmouseover="this.style.background='rgba(16,185,129,0.15)'" onmouseout="this.style.background='rgba(16,185,129,0.08)'">
      ★ {{Certificate Link}}
    </div>
  `;
