...
<div class="mse-props-section">
  ...
  <div class="mse-prop-row" style="margin-top:8px">
    ...
    ${fields.map((f, i) => msc_itemCard(bid, 'fields', i, fields.length, f.label || `Field ${i + 1}`, `
      ${msc_miniField('Label', f.label, `msc_setItem('${bid}','fields',${i},'label',this.value)`, 'text', 'Field label')}
      <div style="margin-bottom:8px">
        ...
        <select class="mse-prop-select" onchange="msc_setItem('${bid}','fields',${i},'type',this.value)" style="font-size:12.5px">
          ${FIELD_TYPES.map(t => `<option value="${t}" ${f.type === t ? 'selected' : ''}>${t}</option>`).join('')}
        </select>
      </div>
      ${msc_miniField('Placeholder', f.placeholder, `msc_setItem('${bid}','fields',${i},'placeholder',this.value)`, 'text', 'Hint text…')}
      ${f.type === 'select' ? msc_miniField('Options (comma-separated)', (f.options || []).join(','), `msc_setItem('${bid}','fields',${i},'options',this.value.split(',').map(s=>s.trim()))`, 'text', 'Option A, Option B') : ''}
      <div style="display:flex;align-items:center;justify-content:space-between;margin-top:4px">
        ...
        <div class="mse-toggle ${f.required ? 'on' : ''}" style="width:32px;height:18px"
          onclick="msc_setItem('${bid}','fields',${i},'required',${!f.required})"></div>
      </div>
    `)).join('')}
